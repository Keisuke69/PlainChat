...
| 永続化 | **SQLite + Drizzle ORM（better-sqlite3）** | ローカル単一マシン用途に最適。単一ファイル DB で外部サービス不要。コード生成不要の軽量 ORM。Drizzle Studio で履歴を GUI 閲覧可。※当初は Prisma 採用、後に Drizzle へ移行（[ADR-007](./ADR.md)）。 |
| 認証 | **Better Auth（email & password）** | TypeScript ネイティブ。ユーザーごとアカウント + 履歴分離をメール/パスワードで標準サポート。Drizzle 連携（`drizzleAdapter`）。 |
...
### 3.1 API 実行方法（トランスポート）の切替【追加】

LLM アクセスは既定の **Vercel AI SDK** に加え、**各社の公式 SDK（`openai` / `@anthropic-ai/sdk`）で API を直接呼び出す「直接」経路**を選べる（画面ヘッダーの「実行方法」で切替、会話ごとに保存）。SDK の抽象化を挟まない素の API 挙動を検証したいときに使う。直接経路は出力を AI SDK の UI message stream へ橋渡しするため、クライアント（`useChat`）は無改修。送信パラメータの正規化はレジストリ（§2）を両経路で共有する。判断の詳細（生 HTTP / 公式 SDK / LiteLLM の比較と不採用理由、永続化方針）は [`ADR.md` の ADR-006](./ADR.md) を参照。
...
PlainChat/
  ...
  drizzle.config.ts                 # drizzle-kit 設定（schema/out/dbCredentials）
  drizzle/                          # 生成されたマイグレーション SQL（drizzle-kit generate）
  scripts/
    seed.ts                         # 初期ユーザー作成（db:seed）
    setup-env.mjs                   # .env 自動生成
  src/
    app/
    ...
    lib/
      ...
      schema.ts                     # ★Drizzle スキーマ（User/Session/Account/Verification + Conversation/Message/ProviderKey）
      db.ts                         # DB クライアント（better-sqlite3 + Drizzle、FK pragma、dev シングルトン）
      auth.ts                       # Better Auth サーバ設定（drizzleAdapter）
...
リポジトリルートの `.gitignore` に `PlainChat/node_modules/`、`PlainChat/.next/`、`PlainChat/.env`、`PlainChat/*.db*`（リポジトリ直下の `dev.db`）を追記する。

---

## 5. データモデル（Drizzle schema 概略 / `src/lib/schema.ts`）
...
2. `.env` に `BETTER_AUTH_SECRET`(ランダム文字列) / `BETTER_AUTH_URL="http://localhost:3000"` / `ENCRYPTION_KEY`(32 バイトのランダム鍵) / `DATABASE_URL="file:./dev.db"` を設定（API キーは UI から入れるので任意）
3. `npm run db:migrate` → `npm run db:seed`（初期ユーザー作成）
4. `npm run dev` → `http://localhost:3000`
5. 動作確認チェックリスト:
   - 初期ユーザーでログインできる / 未ログインはチャットにアクセスできない
   - 設定画面で OpenAI / Anthropic の API キーを入力 → 保存できる。再読込しても「設定済み（下 4 桁マスク）」が保持され、生キーはレスポンスに含まれない。`ProviderKey` が暗号化保存されている（`npm run db:studio` で `encryptedKey` が平文でないこと）
   ...
   - 送信後にリロードしても会話と履歴が残る（`npm run db:studio` で確認）
