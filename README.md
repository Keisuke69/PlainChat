...
- **API リクエストの実行方法を切替**（Vercel AI SDK / プロバイダー公式SDK 直接）。会話ごとに保存され、後から再現できる
...
- 直接経路用に各社公式 SDK（`openai` / `@anthropic-ai/sdk`）も同梱（「実行方法」で切替）
...
## API 実行方法（トランスポート）の切替

画面ヘッダーの「実行方法」で、API リクエストの送り方を切り替えられます。

- **Vercel AI SDK**（既定）: 本番プロダクトと同じ `streamText` 経由。
- **プロバイダー公式SDK（直接）**: `openai` / `@anthropic-ai/sdk` で各社 API を直接呼び出します。Vercel AI SDK の抽象化を挟まないため、**API の素の挙動をより忠実に確認したいとき**に使えます。

どちらの経路でも、送信パラメータは `src/lib/model-registry.ts` の `supports` に従って正規化されます（例: **Claude Opus 4.8 / 4.7 へ temperature / top_p を送らない**）。選択は**会話ごとに保存**され、履歴を開くと復元されます。API キーはどちらの経路でも同じものを使います。

## 便利コマンド

```bash
npm run db:studio   # Drizzle Studio で履歴データを GUI 閲覧
npm run db:generate # スキーマ変更時にマイグレーションSQLを生成
